  ...
  return (
    <div className="space-y-6 p-6">
      <div>
        ...
        <p className="text-slate-500">Sell from inventory and optionally record an internal payment.</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="xl:col-span-2">
              ...
              <div className="mt-2 grid gap-2 md:grid-cols-[1fr_1.4fr]">
                <div className="relative">
                  ...
                  <Input value={customerSearch} onChange={(event) => setCustomerSearch(event.target.value)} placeholder="Search customer" className="pl-8" />
                </div>
                <select {...register("customerId")} className="h-8 rounded-lg border border-input bg-white px-3 text-sm">
        ...
        <Card className="rounded-lg">
          <CardHeader className="gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">Stock is deducted after the invoice is saved.</p>
            </div>
            <div className="flex flex-col gap-2 sm:flex-row">
              ...
              <Button type="button" variant="outline" onClick={() => append(emptyItem)}><Plus className="size-4" />Add Row</Button>
            </div>
          </CardHeader>
          <CardContent className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="min-w-72">Inventory</TableHead>
                ...
                {fields.map((field, index) => {
                  ...
                  return (
                    <TableRow key={field.id}>
                      <TableCell>
                        <select value={watchedItems[index]?.inventoryId || ""} onChange={(event) => handleInventoryChange(index, event.target.value)} className="h-8 w-full rounded-lg border border-input bg-white px-3 text-sm">
                          <option value="">{inventoryLoading ? "Loading inventory..." : "Select inventory item"}</option>
                          {inventoryItems.map((item) => (
                            <option key={item.id} value={item.id}>{item.medicine?.name} {item.medicine?.strength ? `- ${item.medicine.strength}` : ""} | Stock {item.stockQuantity}</option>
                          ))}
                        </select>
                      ...
                      <TableCell>{selected?.stockQuantity ?? "-"}</TableCell>
                      <TableCell><Input type="number" min={1} {...register(`items.${index}.quantity`)} className="w-24" /></TableCell>
                      <TableCell><Input type="number" step="0.01" min={0} {...register(`items.${index}.unitPrice`)} className="w-28" /></TableCell>
                      <TableCell><Input type="number" step="0.01" min={0} {...register(`items.${index}.vat`)} className="w-28" /></TableCell>
                      <TableCell><Input type="number" step="0.01" min={0} {...register(`items.${index}.discount`)} className="w-28" /></TableCell>
                      <TableCell className="font-semibold">{money(rowTotal)}</TableCell>
                      <TableCell className="text-right">
                        <Button type="button" variant="destructive" size="icon" onClick={() => remove(index)} disabled={fields.length === 1} aria-label="Remove row">
        ...
        <div className="flex justify-end gap-3">
          <Button type="button" variant="outline" asChild><Link href="/pharmacy/invoices">Cancel</Link></Button>
          <Button type="submit" disabled={isSubmitting || createInvoice.isPending}>{isSubmitting || createInvoice.isPending ? "Saving..." : "Save Invoice"}</Button>
        </div>
      </form>
    </div>
  );
