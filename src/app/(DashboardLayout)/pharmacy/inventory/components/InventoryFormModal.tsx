    ...
    <div className="flex items-center justify-between border-b border-(--color-border) bg-(--color-bg-secondary) px-6 py-4">
      <div>
        <h2 className="text-lg font-semibold text-(--color-text)">
          {isEditing ? "Edit Inventory Item" : "Add Medicine to Inventory"}
        </h2>
        <p className="text-sm text-(--color-text-muted)">
          Use medicines from the admin-managed global database.
        </p>
      </div>
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="overflow-y-auto p-6">
      <div className="grid gap-5 md:grid-cols-2">
        <div className="md:col-span-2">
          ...
          <div className="mt-2 grid gap-3 md:grid-cols-[1fr_2fr]">
            ...
            <select
              {...register("medicineId")}
              disabled={isEditing}
              className="h-8 rounded-lg border border-(--color-border) bg-(--color-surface) px-3 text-sm text-(--color-text) outline-none focus-visible:ring-2 focus-visible:ring-(--color-primary) disabled:opacity-60"
            >
          ...
          {errors.medicineId && <p className="mt-1 text-xs text-(--color-danger)">{String(errors.medicineId.message)}</p>}
        </div>
        ...
        <div>
          ...
          {errors.stockQuantity && <p className="mt-1 text-xs text-(--color-danger)">{String(errors.stockQuantity.message)}</p>}
        </div>
        ...
        <div>
          ...
          {errors.sellingPrice && <p className="mt-1 text-xs text-(--color-danger)">{String(errors.sellingPrice.message)}</p>}
        </div>
        ...
        <div>
          ...
          <select
            id="status"
            {...register("status")}
            className="mt-2 h-8 w-full rounded-lg border border-(--color-border) bg-(--color-surface) px-3 text-sm text-(--color-text) outline-none focus-visible:ring-2 focus-visible:ring-(--color-primary)"
          >
      ...
      <div className="mt-6 rounded-lg border border-(--color-border) bg-(--color-bg-secondary) p-4">
        ...
        <Textarea
          ...
          className="mt-2 min-h-20 bg-(--color-surface) text-(--color-text)"
        />
      </div>
